...
/// <summary>
/// FAZ35.90: Yeni 03 admin sahnesi (idx 2) için bonus oyun bilgi HUD'u.
/// TutorialBonusHUD.cs (idx 3) emsali — sadece BUILD_INDEX 3→2 ve sınıf/namespace adı değişti.
/// Tutorial pedagojik akışı korunur (TutorialBonusHUD dokunulmadı, ayrı sınıf olarak yaşar).
/// Polling: OyunYoneticisi.{BonusAktifMi, BonusHakKalan, OturumKazanc} public field/property.
/// Konum: sağ-orta, 280×180 px (görev paneli ile aynı X sütunu, altında 12px boşluk),
/// altın çerçeve + koyu zemin + pulse animasyonu.
/// Awake guard: buildIndex != 2 → SetActive(false). Diğer sahneler (02 anlatıcı idx 1, eski 04 Tutorial idx 3) etkilenmez.
/// </summary>
[Preserve]
public class AdminBonusHUD : MonoBehaviour
{
    public const int ADMIN_SAHNE_BUILD_INDEX = 2; // FAZ35.90: Yeni 03 admin sahnesi.
    public const int CANVAS_SORTING_ORDER = 1640;

    public static AdminBonusHUD Ornek { get; private set; }

    // Görev Paneli ile aynı X sütununda, altında — 280×180
    ...
    // Renkler — TutorialBonusHUD ile uyumlu
    ...
    private static void OtomatikInit()
    {
        ...
        if (aktifSahne.buildIndex == ADMIN_SAHNE_BUILD_INDEX)
    ...
    {
        if (scene.buildIndex != ADMIN_SAHNE_BUILD_INDEX)
        {
            if (Ornek != null) UnityEngine.Object.Destroy(Ornek.gameObject);
            return;
        }
        if (Ornek != null) return;
        var go = new GameObject(nameof(AdminBonusHUD));
        go.AddComponent<AdminBonusHUD>();
    }

    private void Awake()
    {
        if (SceneManager.GetActiveScene().buildIndex != ADMIN_SAHNE_BUILD_INDEX)
        {
        ...
        Debug.Log("[AdminBonusHUD] Spawn: Ornek atandı, UIYarat tamamlandı.");
    }
    ...
    // === Pulse animasyon (TutorialBonusHUD ile aynı) ===
    ...
    // === UI yaratımı — SAĞ ORTA (TutorialBonusHUD aynası) ===

    private void UIYarat()
    {
        _root = new GameObject("AdminBonusHUDCanvas",
        ...
        // Panel — Görev Paneli ile aynı X sütununda (anchor 0.5/0.5 pos 801,222 size 280×320 emsali) altında.
        ...
        _panelRt.anchoredPosition = new Vector2(801f, 50f); // X (sağ kenar bölgesi), Y=50 (üst kenar)
        ...
        spinRt.offsetMin = new Vector2(10f, -25f);
