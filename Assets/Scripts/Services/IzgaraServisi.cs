...
public class IzgaraServisi
{
        ...
        for (int idx = 0; idx < total; idx++)
        {
            ...
            if (_grid[x, y] == CARPAN_SEMBOL)
            {
                ...
                else
                {
                    ...
                    // CarpanTextDegeriCoz hem "xN" hem "Nx" formatını destekler — geriye uyumlu.
                    if (CarpanTextDegeriCoz(tmp.text) > 0)
                    {
                        if (!tmp.gameObject.activeSelf) tmp.gameObject.SetActive(true);
                        var c = tmp.color;
                        c.a = 1f;
        ...
        for (int y = 0; y < _satir; y++)
        {
            for (int x = 0; x < _sutun; x++)
            {
                ...
                if (sym == CARPAN_SEMBOL)
                {
                    ...
                    img.rectTransform.localScale = new Vector3(1.5f, 1.5f, 1f); // bomba 1.5x — yan meyvelerden baskın
                    Debug.Log($"[IzgaraScale] RenderAllSprites SET çarpan 1.5x cell={idx}, scale={img.rectTransform.localScale}");
                    ...
                    if (_carpanHücreTextleri != null && idx < _carpanHücreTextleri.Length && _carpanHücreTextleri[idx] != null)
                    {
                        ...
                        if (v > 0)
                        {
                            SetCarpanText(_carpanHücreTextleri[idx], v);
                            _carpanHücreTextleri[idx].gameObject.SetActive(true);
                        }
                        else
                        {
                            // Bomba görünüyorsa yazıyı bir karede düşürme; mevcut çarpan değeri varsa koru.
                            // CarpanTextDegeriCoz hem "xN" hem "Nx" formatını destekler.
                            var mevcut = _carpanHücreTextleri[idx].text;
                            bool korunacak = CarpanTextDegeriCoz(mevcut) > 0;
                ...
                else
                {
                    ...
                    img.rectTransform.localScale = Vector3.one; // normal meyve → scale reset
                ...
                if (resetScale)
                {
                    img.rectTransform.localScale = (sym == CARPAN_SEMBOL)
                        ? new Vector3(1.5f, 1.5f, 1f)  // çarpan 1.5x kalır
                        : Vector3.one;                  // normal meyve reset
                    if (sym == CARPAN_SEMBOL)
                        Debug.Log($"[IzgaraScale] RenderAllSprites RESET-block KORUMA çarpan 1.5x cell={idx}");
                }
    ...
    private static int CarpanTextDegeriCoz(string text)
    {
        ...
        // İki format desteklenir: "x{N}" (eski) ve "{N}x" (yeni). Hangisi varsa parse et.
        char ilk = t[0];
        char son = t[t.Length - 1];
        string sayiKismi;
        if (ilk == 'x' || ilk == 'X')
            sayiKismi = t.Substring(1);                     // eski: "x100" → "100"
        else if (son == 'x' || son == 'X')
            sayiKismi = t.Substring(0, t.Length - 1);       // yeni: "100x" → "100"
        else
            return 0;
        if (int.TryParse(sayiKismi, out int v) && v > 0)
            return v;
        return 0;
    }

    /// <summary>Çarpan değerine göre düz renk: ×2 sarı, ×3-5 turuncu, ×8+ altın.</summary>
    public static Color CarpanRengiAl(int deger)
    {
        if (deger <= 2) return new Color(1f, 0.878f, 0.4f);          // sarı  #FFE066
        if (deger <= 5) return new Color(0.984f, 0.573f, 0.235f);    // turuncu #FB923C
        return new Color(1f, 0.843f, 0f);                            // altın #FFD700
    }

    /// <summary>Carpan TMP'sini değer + beyaz renk + siyah outline ile set eder. Format: "{deger}x"
    /// (sayı önce, x sonra). enableVertexGradient kapatılır (düz renk için override).
    /// Beyaz yazı + siyah outline → maksimum kontrast, bombanın renkli zemininde net okunur.
    /// CarpanRengiAl helper geriye dönüş için orphan tutulur (renkli paleti gerektiğinde tekrar kullanım).
    /// IzgaraServisi'nin 3 yerinde + CarpanOverlayServisi'nin 2 yerinde merkezi kullanım.</summary>
    public static void SetCarpanText(TextMeshProUGUI tmp, int deger)
    {
        if (tmp == null) return;
    ...
    public virtual void RenderSpritesOnlyForCells(IEnumerable<Vector2Int> hucreler, int[,] grid)
    {
        ...
        foreach (var p in hucreler)
        {
            ...
            if (sym == CARPAN_SEMBOL)
            {
                ...
                img.rectTransform.localScale = new Vector3(1.5f, 1.5f, 1f); // bomba 1.5x
                Debug.Log($"[IzgaraScale] RenderSpritesOnlyForCells SET çarpan 1.5x cell={idx}, scale={img.rectTransform.localScale}");
            ...
            img.rectTransform.localScale = (sym == CARPAN_SEMBOL)
                ...
                : Vector3.one;                  // normal meyve reset
            if (sym == CARPAN_SEMBOL)
                Debug.Log($"[IzgaraScale] RenderSpritesOnlyForCells END KORUMA çarpan 1.5x cell={idx}");
        }
    }
    ...
    public virtual void EnsureCarpanCellTexts()
    {
        ...
        for (int i = 0; i < _hucreler.Length; i++)
        {
            ...
            rt.localScale = new Vector3(1.5f, 1.5f, 1f); // 1.5x büyütme — daha belirgin görünüm
