...
public class CarpanServisi
{
    ...
    private bool _buSpindeCarpanYerlesti;
    ...
    public virtual void SetIsCarpanUretimiAktif(Func<bool> fn) => _isCarpanUretimiAktif = fn;
    public virtual void SetIsCarpanSadeceBonus(Func<bool> fn) => _isCarpanSadeceBonus = fn;
    public virtual void SetGetCarpanUretimOlasiligi(Func<float> fn) => _getCarpanUretimOlasiligi = fn;
    public virtual void SetGetMaxCarpanAdedi(Func<int> fn) => _getMaxCarpanAdedi = fn;
    public virtual void SetRollCarpanDegeri(Func<int> fn) => _rollCarpanDegeri = fn;
    public virtual void SetGetSpinKazancHam(Func<int> fn) => _getSpinKazancHam = fn;
    public virtual void SetGetBonusRemainingPayableTL(Func<int> fn) => _getBonusRemainingPayableTL = fn;
    public virtual void SetForceCarpan(int value) => _forceCarpan = Mathf.Max(0, value);

    public virtual void ResetForNewSpin(int maxCarpanAdedi)
    {
        _spinCarpanCarpim = 0;
        _spinCarpanDegerleri.Clear();
        _pendingCarpanDusurecek.Clear();
        _carpanKalanBuSpin = Mathf.Max(0, maxCarpanAdedi);
        _buSpindeCarpanYerlesti = false;
    }

    public virtual long GetCurrentMultiplier()
    {
    ...
    public virtual int GetTotalMultiplierForSpin()
    {
    ...
    public virtual bool TryScheduleCarpanDrop(bool bonusAktif)
    {
        // Kural: Aynı spin içinde ikinci turda yeni bomba planlama.
        if (_buSpindeCarpanYerlesti) return false;

        bool carpanUretimiAktif = _isCarpanUretimiAktif == null || _isCarpanUretimiAktif();
        bool sadeceBonus = _isCarpanSadeceBonus != null && _isCarpanSadeceBonus();
        int force = _forceCarpan;
        _forceCarpan = 0;

        // Zorla çarpan her zaman öncelikli.
        if (force > 0)
        {
    ...
    public virtual IReadOnlyList<int> GetPendingDrops() => _pendingCarpanDusurecek;

    public virtual void ClearPendingDrops() => _pendingCarpanDusurecek.Clear();

    public virtual void RecordPlacedCarpanlar(IReadOnlyList<int> placed)
    {
        if (placed == null) return;
        if (placed.Count > 0) _buSpindeCarpanYerlesti = true;
    ...
    public virtual int GetCarpanKalanBuSpin() => _carpanKalanBuSpin;
}
...
public class CarpanYerlestirmeServisi
{
    ...
    public virtual void SetBaglam(ICarpanYerlestirmeBaglami ctx)
    {
        _ctx = ctx;
    }

    public virtual void CarpanlariDoluGriddeUygula()
    {
        if (_ctx == null) return;
