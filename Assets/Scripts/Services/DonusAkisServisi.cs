...
public class DonusAkisServisi
{
    ...
    public IEnumerator NormalSpinAkisi()
    {
        ...
        // İSTİSNA: A5 Spin 4 cazip bonus tuzağı (sonKayit.bonusOyunuTetikle=true) → horn'u atla,
        // pedagojik olarak orada "sahte kutlama" (alkış + havai fişek) çalacak.
        if (hamKazanc == 0)
        {
            bool a5BonusTetik = false;
            if (Senaryo.Scripted.ScriptedSpinYoneticisi.Aktif
                && Senaryo.Scripted.ScriptedSpinYoneticisi.Ornek != null)
            {
                var k = Senaryo.Scripted.ScriptedSpinYoneticisi.Ornek.SonKayit;
                if (k != null && k.bonusOyunuTetikle) a5BonusTetik = true;
            }
            if (!a5BonusTetik)
                _ctx.PlayKayipHorn();
            else
                Debug.Log("[KayipHorn] A5 cazip popup açılacak — horn baskılandı (sahte kutlama yapılacak)");
        }
