...
{
    /// <summary>
    /// PAKET 1: TutorialModalKopru'nun TAM KOPYASI — tutorial sahnesi (build idx 3) için bağımsız modal.
    /// Sadece namespace, class adı ve build idx sabiti değişti; davranış aynı (typewriter + click + slide).
    ...
    /// Sadece sahne 3 (04_AdminOyunScene)'de aktif.
    /// </summary>
    [Preserve]
    public class TutorialModalKopru : MonoBehaviour
    {
        ...
        {
            if (scene.buildIndex != TUTORIAL_SAHNE_BUILD_INDEX) return;
            if (Ornek != null) return;
            var go = new GameObject(nameof(TutorialModalKopru));
            go.AddComponent<TutorialModalKopru>();
        }
        ...
        private void UIYarat()
        {
            ...
            _root = new GameObject("TutorialModalKopruCanvas",
        ...
        private static Sprite EgitmenGorseliniAl()
        {
            ...
            if (sprite != null)
            {
                Debug.Log("[TutorialModalKopru] Resources/egitmenyuz.png yüklendi.");
                _cachedSprite = sprite;
                return sprite;
            }

            Debug.LogWarning("[TutorialModalKopru] Resources/egitmenyuz.png bulunamadı (Sprite import edilmemiş olabilir) — procedural eğitmen silueti kullanılıyor.");
