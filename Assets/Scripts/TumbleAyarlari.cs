...
public class TumbleAyarlari : MonoBehaviour
{
    ...
    // Pay table: SB’ye yakın, ~%10 cimri (×0,9). Index sırası Inspector’daki sprite sırasıyla aynı olmalı; 0 TL küme = dolandırıcılık hissi → sadece scatter 0.
    ...
    public virtual void EnsurePayTablesInitialized(int sembolSayisi)
    {
    ...
    public virtual float GetPayForCount(int sembolIndex, int eslesmeSayisi)
    {
    ...
    [Min(0f)] public float PopDuration = 0.38f;

    [Tooltip("Dusme animasyon suresi (tumble dolum; cok dusuk = meyveler cok hizli iner)")]
    [Min(0f)] public float FallDuration = 0.82f;

    [Tooltip("Adimlar arasi bekleme")]
    [Min(0f)] public float BetweenStepsDelay = 0.18f;
    ...
    public virtual List<Sprite> GetSembolSpriteListesi()
    {
    ...
    public virtual void SetZorluk(int v)
    {
    ...
    public virtual void SetScatterAyarlari(int spriteIndex, float normalChance, float bonusChance, int esik, int maxScatter)
    {
    ...
    public virtual int ScatterSayMethod(int[,] gridData, int satir, int sutun)
    {
    ...
    public virtual float CurrentScatterChance(bool bonusAktif) => bonusAktif ? _scatterChanceBonus : _scatterChanceNormal;

    // OyunYoneticisi'den çağrılan versiyon
    public virtual int RandomSymbolWithScatterChance(int[,] grid, bool bonusAktif, int satir, int sutun)
    {
    ...
    public virtual int RandomSymbolWithScatterChanceForGrid(int[,] gridData, bool bonusAktif, int satir, int sutun)
    {
        if (gridData == null) return 0;
    ...
    public virtual float BiasMultiplier(float easyMult, float hardMult)
    {
        float m = 1f;
    ...
    public virtual int RandomNonScatterSymbol(int[,] gridData, int satir, int sutun)
    {
    ...
    public virtual void FillRandomAll(int[,] grid, int[,] carpanDegerGrid, int[] carpanDegerByCellIndex, int satir, int sutun, bool bonusAktif, int maxTry = 30)
    {
        if (grid == null) return;
