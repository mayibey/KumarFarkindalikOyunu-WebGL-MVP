...
namespace Senaryo.Scripted
{
    ...
    public class ScriptedBonusTuzagiPopup : MonoBehaviour
    {
        ...
        [DllImport("__Internal")] private static extern void HavaiFisekDurdur();
#endif
        ...
        public IEnumerator PopupGoster(int bakiye, OyunYoneticisi oy)
        {
            ...
            // Sahte kutlama (pedagojik tuzak): alkış sesi + DOM canvas üzerinde gerçek havai fişek
            // (PanelBridge.jslib > HavaiFisekBaslat). A5 Spin 4'te brüt 0 olmasına rağmen oyun
            // "kutluyor" gibi davranır → sahte umut/euforyaya kapılma riskini kullanıcıya hissettirir.
            // Horn (kayıp sesi) bu yolda DonusAkisServisi'nde baskılanır.
            if (oy != null && oy.bonusEndSfxSource != null && oy.bonusEndApplauseClip != null)
                oy.bonusEndSfxSource.PlayOneShot(oy.bonusEndApplauseClip);
#if UNITY_WEBGL && !UNITY_EDITOR
            try { HavaiFisekBaslat(); }
            catch (System.Exception e) { Debug.LogWarning("[HavaiFisek] Başlat hatası: " + e.Message); }
#endif
            Debug.Log("[CazipPopup] Sahte kutlama: alkış + havai fişek (DOM) tetiklendi");
            ...
            // Havai fişek loop'u durdur (yeni roket üretmesin; mevcutlar bitince DOM canvas otomatik kalkar)
#if UNITY_WEBGL && !UNITY_EDITOR
            try { HavaiFisekDurdur(); }
            catch { /* sessiz */ }
#endif
        ...
        // Havai fişek artık DOM/Canvas tabanlı (PanelBridge.jslib > HavaiFisekBaslat/Durdur).
        // Unity tarafında ayrı bir partikül üretimi yok.
