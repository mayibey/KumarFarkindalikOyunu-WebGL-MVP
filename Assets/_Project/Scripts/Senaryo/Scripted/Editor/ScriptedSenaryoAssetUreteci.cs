...
namespace Senaryo.Scripted.Editor
{
    ...
    public static class ScriptedSenaryoAssetUreteci
    {
        ...
        private const string M_A2_S2 =
            "<color=#ef4444><b>Dikkat: Manipülasyon</b></color>\n" +
            "Oyuncu az önce <color=#60a5fa><b>1.500 TL</b></color> <color=#fb923c><b>bahis</b></color> yapar. Ekranda \"kazanç 750 TL\" ifadesi görünür; ancak bakiyeden <color=#ef4444><b>750 TL eksilir</b></color>. Buna rağmen oyuncunun zihninde <color=#fb923c><b>\"kazandım\" hissi</b></color> oluşur. Bu durum, <color=#60a5fa><b>sistem</b></color> tarafından bilerek tasarlanan bir etkiyi gösterir.\n\n" +
            "Buradaki amaç, oyuncuya <color=#ef4444><b>kaybettiğini</b></color> hissettirmeden sürekli kazanç elde ediyormuş algısını oluşturmaktır. Her <color=#60a5fa><b>spinde</b></color> yatırılan miktardan daha düşük ödeme yapılmasına rağmen ekranda büyük puntolarla \"kazanç\" ifadesi gösterilir.\n\n" +
            "Uzun vadede oyuncu daima <color=#ef4444><b>zarar eder</b></color>. <color=#60a5fa><b>Sistem</b></color> bunu <color=#ef4444><b>bilinçli</b></color> olarak tasarlar; bakiyenin sürekli artıyormuş gibi gösterilmesi yoluyla oyuncunun oyunda kalması sağlanır ve temel hedef bu davranışın sürdürülmesidir.";
        ...
        private const string M_A2_S4 = "Oyuncu <color=#60A5FA>oyunu yönettiğini</color> düşünürken, oyun onu <color=#EF4444>adım adım içine çeker</color>.";
        // A2 Spin 6: kontrol yanılsaması pekişmesi (3. tekil)
        private const string M_A2_S6 = "Ekrana hem üzüm hem de elmadan <color=#2563eb>7'şer adet</color> düşer. <color=#2563eb>8 adet</color> olması halinde oyuncu oyunu kazanmış olacaktı. Her iki sembol de kıl payı kaçırılmış olur.\n\nOyuncu bu durumda <color=#ea580c>\"çok yakındım, bir daha denesem kesin olur\"</color> düşüncesine kapılır. Bu düşünce, <color=#dc2626>manipülatif bir etki</color> olarak değerlendirilir. <color=#2563eb>Sistem</color>, bu tür yakın sonuç durumlarını <color=#dc2626>bilinçli biçimde</color> üretir.\n\n<color=#2563eb>Kontrol algısı</color> bu şekilde güçlendirilir ve oyuncunun oyuna devam etme eğilimi artırılır.";
        // A2 Spin 8 modal KALDIRILDI (sade akış).

        // A3 modal mesajları
        private const string M_A3_S3 = "İlk ciddi <color=#dc2626>kayıplar</color> yaşanmaya başlar. Oyuncunun amacı para kazanmaktan çıkar ve <color=#2563eb>kayıpları telafi etmeye</color> dönüşür.";
        ...
        private const string M_A3_S6 = "Oyuncu <color=#ef4444><b>kayıpları</b></color> geri kazanmak için daha fazla <color=#ef4444><b>risk alır</b></color>. Mantıklı düşünme yetisini kaybeder.\n\nŞimdi oyuncu <color=#fb923c><b>\"yüksek bahis daha hızlı kurtarır\"</b></color> yanılgısıyla <color=#fb923c><b>bahis</b></color> miktarını <color=#60a5fa><b>2500 TL</b></color>'ye yükseltecektir. Bu da <color=#60a5fa><b>algoritmanın</b></color> aslında istediği davranış biçimidir.";
        // A3 Spin 7
        ...
        private const string M_A4_S2 = "Üst üste gelen <color=#ef4444><b>kayıplar</b></color> oyuncuyu yıpratır. <color=#60a5fa><b>Algoritma</b></color>, birkaç <color=#60a5fa><b>spin</b></color> sonrasında büyük bir kazanç planlar. Bu durum, oyuncuda <color=#fb923c><b>\"şansım döndü\"</b></color> algısı oluşturur.\n\nAncak <color=#60a5fa><b>sistem</b></color>, bu noktaya gelmeden önce oyuncuyu pes etme eşiğine kadar sürükler.";
        private const string M_A4_S4 = "Oyuncu ciddi <color=#dc2626>kayıplar</color> yaşadığı için oyundan çıkmak üzeredir. Tam bu noktada <color=#2563eb>algoritma</color>, <color=#16a34a>büyük bir kazanç</color> vererek oyuncunun <color=#ea580c>\"şansının döndüğü\"</color> algısına kapılmasını sağlar.";
        // A4 Spin 5 modali asset'ten kaldırıldı — AnlaticiSeritKopru.A4S5CarpanModalAkisi runtime'da çağırır.

        // A5 modal mesajları
        private const string M_A5_S1 = "<color=#ea580c>Şansının döndüğünü</color> düşünen oyuncu, <color=#16a34a>bakiyesinin</color> arttığını gördükçe <color=#ea580c>bahis</color> miktarını daha da yükseltir ve daha fazla kazanmayı hedefler. <color=#ea580c>Bahis</color> miktarındaki artış, oyuncuda <color=#ea580c>adrenalin</color> seviyesinin yükselmesine neden olur.";
        // A5 Spin 3: ×500 çarpan kaçtı + sabırsızlık silahı
        private const string M_A5_S3 = "Oyunda <color=#2563eb>500x çarpanı</color> düşer; ancak ekranda aynı sembolden <color=#2563eb>8 adet</color> bulunmadığı için herhangi bir ödeme yapılmaz. Bu durum, <color=#2563eb>sistem</color> tarafından oyuncuyu bir sonraki aşamaya yönlendirmek için kullanılan bir <color=#dc2626>yem</color> niteliği taşır.\n\nOyuncu <color=#2563eb>500x çarpanı</color> gördüğünde <color=#ea580c>şansının döndüğüne</color> inanır ve bu güven duygusuyla bir sonraki <color=#dc2626>tuzağa</color> yönlendirilir.";
        ...
        // === Bahis sabitleri (AnlaticiSeritKopru._onerilenBahisler ile BİREBİR senkron) ===
        ...
        // A5 cazip popup → bonus oyun: 1000 TL bahis × 10 spin = 10000 ekvivalenti yatırım hissi
        ...
        public static void AssetiYenidenUret()
        {
            ...
            // A5 cazip popup → bonus oyun (10 sabit spin, toplam 4000 TL).
            asset.bonusSpinleri.Clear();
            DoldurBonusSpinleri(asset.bonusSpinleri);
        ...
        {
            // Spin 1: 3 tumble {hindistan→elma→üzüm} = (0.5+1.0+1.5)×500 = 1500 | modal A1_S1
            {
            ...
            // Spin 7: 8 üzüm + 8 elma + x5 çarpan (MEGA) = (1.5+1.0)×500×5 = 6250
            {
        ...
        private static void DoldurAsama2(List<ScriptedSpinKaydi> liste)
        {
            ...
            // Spin 5: 2 tumble {hindistan→karpuz} = (0.5+0.6)×1500 = 1650 (net +150, tumble görseli)
            {
                ...
                var t1 = TumbleTekDusen(ilk, new[] { SYM_HINDISTAN }, SYM_KARPUZ);
                int[] g1 = GridSonrasiHesapla(ilk, t1);
        ...
        private static void DoldurAsama4(List<ScriptedSpinKaydi> liste)
        {
            ...
            // Spin 5: 8 ARMUT + ilk grid x100 çarpan (MEGA WIN) = 0.2×2500×100 = 50000
            // Faz 35.38: önceden elle 20000 yazılıydı (bahis 1000 varsayımı); BAHIS_A4=2500 olunca
            // paytable formülü doğru 50000 üretir. AnlaticiSeritKopru.A4S5CarpanModalAkisi runtime modal.
            {
                int[] all = { SYM_ARMUT };
        ...
        private static void DoldurAsama5(List<ScriptedSpinKaydi> liste)
        {
            ...
            // Spin 2: 8 üzüm + ilk grid x2 çarpan = 1.5×4000×2 = 12000
            // Faz 35.38: önceden elle 6000 yazılıydı (bahis 2000 varsayımı); BAHIS_A5=4000 ile paytable doğru.
            {
                int[] all = { SYM_UZUM };
            ...
            // Spin 5: cluster yok + tumble yok → formül 0 (Tiyatro paytable hesabı 0 üretir).
            // Faz 35.38: önceden hayalet 800 brüt vardı, asset rakamı runtime'da hiç oluşmuyordu.
        ...
        // ============================================================
        // BONUS — A5 cazip popup → 10 sabit spin (toplam 4000 TL)
        // Pedagojik ritim: 5 sıfır + 5 kazanç (anti-climax kapanış).
        // Bahis 1000 (SCRIPTED_BONUS_BAHIS_TL ile senkron). Paytable doğrulanmış:
        // Spin 2/8: HINDISTAN×8 = 0.5×1000 = 500
        // Spin 4: MUZ×8 = 0.8×1000 = 800
        // Spin 6: ELMA×8 + ARMUT×8 = (1.0+0.2)×1000 = 1200
        // Spin 9: HINDISTAN×10 = 1.0×1000 = 1000 (10-11 paytable)
        // ============================================================
        private static void DoldurBonusSpinleri(List<ScriptedSpinKaydi> liste)
        {
            // asamaIndex=4 (A5'in alt akışı). Seed(7, X) — diğer aşamalarla çakışmasın.

            // Spin 1: 7 UZUM near-miss → 0 (bonus girişi şok)
            ...
            liste.Add(SpinTanimi(10, 4, BAHIS_BONUS, SpinTipi.NearMiss, 0,
                GridIlk(null, Seed(7, 10), (SYM_ERIK, 7)), null, NoTumble()));
        }
        ...
        /// <summary>Tek cluster'lı basit spin: 8 hücre patlar, dolgu sembolleri düşer (kazançsız son state).
        /// Faz 35.38: brut parametresi kaldırıldı — paytable formülü içeride hesaplar (sym×8 × bahis × 1).</summary>
        private static ScriptedSpinKaydi TekClusterSpin(int spinNo, int asama, int bahis, int sym, SpinTipi tip, string modal = null)
        {
            int[] all = { sym };
